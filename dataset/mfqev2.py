...
from utils import FileClient, paired_random_crop, augment, totensor, import_yuv, paired_random_crop_two
...
class UDADataset(data.Dataset):
    ...
    def __init__(self, opts_dict, radius):
        ...
        gt_path_list = sorted(glob.glob(self.gt_root))
...
class SelectorDataset(data.Dataset):
    """MFQEv2 dataset.
    ...
    def __getitem__(self, index):
        ...
        # lqs = [288, 2880]
        change_size = np.random.randint(-5, 5)
        change_size_1 = np.random.randint(-5, 5)
        change_pos = np.random.randint(0, 6)
        if change_pos > 2:
            change_pos += 1
        inter_pos = np.random.randint(10, 2580)
        inter_pos_1 = np.random.randint(10, 100)

        img_lqs_1 = []
        img_lqs_2 = []
        for p in range(len(img_lqs)):
            img_lqs_1.append(img_lqs[p][inter_pos_1:inter_pos_1 + 100, inter_pos:inter_pos + 288, :])
            img_lqs_2.append(img_lqs[p][inter_pos_1:inter_pos_1 + 100, inter_pos:inter_pos + 288, :])
        img_lqs_2[change_pos] = img_lqs[change_pos][inter_pos_1 + change_size_1:inter_pos_1 + 100 + change_size_1,
                                      inter_pos + change_size:inter_pos + 288 + change_size, :]

        img_gt = img_gt[inter_pos_1:inter_pos_1 + 100, inter_pos:inter_pos + 288, :]

        # randomly crop
        img_gt, img_lqs_1, img_lqs_2 = paired_random_crop_two(
            img_gt, img_lqs_1, img_lqs_2, gt_size, img_gt_path
        )

        # flip, rotate
        for p in range(len(img_lqs_2)):
            img_lqs_1.append(img_lqs_2[p])  # gt joint augmentation with lq
        img_lqs_1.append(img_gt)
        img_results = augment(
            img_lqs_1, self.opts_dict['use_flip'], self.opts_dict['use_rot']
        )

        # to tensor
        img_results = totensor(img_results)
        img_lqs_1 = torch.stack(img_results[0:7], dim=0)
        img_lqs_2 = torch.stack(img_results[7:14], dim=0)

        img_gt = img_results[-1]

        return {
            'lq_1': img_lqs_1,  # (T [RGB] H W)
            'lq_2': img_lqs_2,
            'gt': img_gt,  # ([RGB] H W)
        }
...
class StableDataset(data.Dataset):
    """MFQEv2 dataset.
    ...
    def __getitem__(self, index):
        ...
        img_gt = img_gt[:, 1295:1583, ...]
        for zzz in range(len(img_lqs)):
            img_lqs[zzz] = img_lqs[zzz][:, 1295:1583, ...]
        # print(img_gt.shape, img_lqs[0].shape)
...
class Stable_Center(data.Dataset):
    """MFQEv2 dataset.
...
class VideoTestMFQEv2Dataset(data.Dataset):
    ...
    def __init__(self, opts_dict, radius):
        ...
        gt_path_list = sorted(glob.glob(op.join(self.gt_root, 'G8ANewEmpire_0.yuv')))
        self.vid_num = len(gt_path_list)
        for idx_vid, gt_vid_path in enumerate(gt_path_list):
            name_vid = gt_vid_path.split('/')[-1]
            # w, h = map(int, name_vid.split('_')[-2].split('x'))
...
class StableVal(data.Dataset):
    ...
    def __init__(self, opts_dict, radius):
        ...
        for idx_vid, gt_vid_path in enumerate(gt_path_list):
            ...
            for iter_frm in range(nfs):
                lq_indexes = list(range(7 * iter_frm, 7 * (iter_frm + 1)))
                # lq_indexes = list(np.clip(lq_indexes, 0, nfs - 1))
                self.data_info['index_vid'].append(idx_vid)
                self.data_info['gt_path'].append(gt_vid_path)
                self.data_info['lq_path'].append(lq_vid_path)
