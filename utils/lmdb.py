...
from utils.file_io import import_yuv, import_10bityuv
import numpy as np
...
def _read_y_from_10bityuv_worker(video_path, yuv_type, h, w, index_frame, key, compress_level):
    """不要把该函数放到主函数里，否则无法并行。"""
    if h == None:
        w, h = [int(k) for k in op.basename(video_path).split('_')[1].split('x')]  # for MFQEv2 dataset
    img = import_10bityuv(
    ...
    if map_size is None:
        ...
        for iter_img in range(num_img):
            ...
            if img_size > biggest_size:
                ...
                biggest_index = iter_img
        # obtain data size of one image
        _, img_byte, _ = _read_y_from_10bityuv_worker(
            video_path_list[biggest_index],
            yuv_type,
            h,
            w,
