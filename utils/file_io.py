...
def import_10bityuv(seq_path, h, w, tot_frm, yuv_type='420p', start_frm=0, only_y=True):
    ...
    # init
    y_seq = np.zeros((tot_frm, h, w), dtype=np.uint16)
    if not only_y:
        u_seq = np.zeros((tot_frm, hh, ww), dtype=np.uint16)
        v_seq = np.zeros((tot_frm, hh, ww), dtype=np.uint16)

    # read data
    with open(seq_path, 'rb') as fp:
        for i in range(tot_frm):
            fp.seek(int(blk_size * (start_frm + i) * 2), 0)  # skip frames
            y_frm = np.fromfile(fp, dtype=np.uint16, count=y_size).reshape(h, w)
            if only_y:
                y_seq[i, ...] = y_frm
            else:
    ...
    y_seq = (y_seq >> 2).astype(np.uint8)
    if not only_y:
        u_seq = (u_seq >> 2).astype(np.uint8)
        v_seq = (v_seq >> 2).astype(np.uint8)
